...
export async function GET(request: Request, { params }: { params: Promise<{ entity: string }> }) {
  ...
  if (entity === "applications") {
    ...
    do {
      ...
      for (const item of data.items) rows.push([item.application_reference, item.dealer_name, item.amount, item.currency, item.status, item.created_at]);
      page += 1;
    } while (page <= pageCount);
    ...
    users.forEach((user) => rows.push([user.email, user.full_name, user.roles.join(";"), user.is_active, user.created_at]));
    ...
    const dealers = (await listDealers())
      ...
      .sort((a, b) => input.sort === "oldest" ? a.created_at.localeCompare(b.created_at) : b.created_at.localeCompare(a.created_at));
    rows.push(["Galeri", "Slug", "İletişim", "KVKK", "Aktif", "Oluşturulma"]);
    dealers.forEach((dealer) => rows.push([dealer.name, dealer.slug, dealer.contact_email, dealer.privacy_contact_email, dealer.is_active, dealer.created_at]));
  } else {
