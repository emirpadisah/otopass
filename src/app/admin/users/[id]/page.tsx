...
export default async function AdminUserDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!user) notFound();
  return <div><PanelPageHeader eyebrow="Yönetim / Kullanıcı" title={user.full_name || user.email || "Kullanıcı"} description="Rol, galeri üyeliği ve hesap durumunu yönetin." icon={UserCog} actions={<Link href="/admin/users" className={cn(buttonVariants({ variant: "secondary", size: "sm" }), "inline-flex")}><ArrowLeft size={14} /> Listeye dön</Link>} /><div className="mt-4"><UserManageForm user={user} dealers={dealers.map((dealer) => ({ id: dealer.id, name: dealer.name }))} canDelete={roles.includes("super_admin")} /></div></div>;
}
