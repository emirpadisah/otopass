...
const statuses = [{ value: "pending", label: "Bekleyen" }, { value: "offered", label: "Teklif verildi" }, { value: "accepted", label: "Kabul edildi" }, { value: "rejected", label: "Reddedildi" }, { value: "sold", label: "Satıldı" }, { value: "archived", label: "Arşivlendi" }];
...
export default async function AdminApplicationsPage({ searchParams }: { searchParams: Promise<Params> }) {
  ...
  return <div><PanelPageHeader eyebrow="Yönetim / Başvurular" title="Başvurular" description="Tüm galerilerdeki araç başvurularını ve güncel durumlarını izleyin." icon={ClipboardList} meta={<span className="ops-chip">{data.total} kayıt</span>} />
    {raw.deleted === "1" ? <div className="status-alert mt-4" data-tone={raw.cleanup === "pending" ? "warning" : "success"} role="status">{raw.cleanup === "pending" ? "Başvuru silindi. Bazı fotoğraflar arka planda silinmeye devam ediyor." : "Başvuru, ilişkili teklifler ve fotoğraflar kalıcı olarak silindi."}</div> : null}
    <PanelSection className="mt-4" title="Başvuru listesi" description={`${data.total} kayıttan ${data.items.length} tanesi gösteriliyor`} icon={ClipboardList} meta={<ListControls q={input.q} status={input.status} sort={input.sort} pageSize={input.pageSize} statuses={statuses} exportHref={`/api/admin/export/applications?${exportQuery}`} />} contentClassName="ops-section-flush">
