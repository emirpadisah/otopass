...
export function DealerManageForm({ dealer, canDelete }: { dealer: Dealer; canDelete: boolean }) {
  ...
  return <div className="grid gap-4 lg:grid-cols-[1fr_340px]">
    ...
    {canDelete ? <form id={deleteFormId} action={deleteAction} className="panel h-fit space-y-4 border-[var(--danger)] p-5"><input type="hidden" name="dealerId" value={dealer.id} /><h2 className="font-bold text-[var(--danger)]">Kalıcı silme</h2><p className="text-sm text-[var(--text-muted)]">Galeriye bağlı başvurular, teklifler ve üyelikler geri alınamaz biçimde silinir.</p>{deleteState.message ? <div className="status-alert" data-tone={deleteState.ok ? "success" : "danger"} role={deleteState.ok ? "status" : "alert"}>{deleteState.message}</div> : null}<ConfirmSubmitButton formId={deleteFormId} title="Galeri kalıcı olarak silinsin mi?" description={`${dealer.name} ve bu galeriye bağlı tüm veriler sistemden kaldırılacak.`} confirmLabel="Galeriyi kalıcı olarak sil" details={["Başvurular, teklifler ve araç fotoğrafları silinir", "Galeri logosu ve kullanıcı üyelikleri kaldırılır", "Bu işlem geri alınamaz"]} feedbackMessage={deleteState.message} feedbackTone={deleteState.ok ? "success" : "danger"} tone="danger" variant="danger" disabled={deletePending}><Trash2 size={15} /> Galeriyi sil</ConfirmSubmitButton></form> : null}
  </div>;
}
