...
export function ListControls({ q, status, sort = "newest", pageSize = 25, statuses = [], exportHref }: { q?: string; status?: string; sort?: string; pageSize?: number; statuses?: Array<{ value: string; label: string }>; exportHref?: string }) {
  return <form className="ops-list-controls" method="get">
    <label className="ops-search"><Search size={15} aria-hidden="true" /><span className="sr-only">Ara</span><input name="q" defaultValue={q} placeholder="Ara..." /></label>
    {statuses.length ? <select name="status" defaultValue={status || ""} className="input-base"><option value="">Tüm durumlar</option>{statuses.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select> : null}
    <select name="sort" className="input-base" defaultValue={sort}><option value="newest">En yeni</option><option value="oldest">En eski</option></select>
    ...
    <button className={cn(buttonVariants({ size: "sm" }), "inline-flex")} type="submit">Filtrele</button>
    {exportHref ? <Link href={exportHref} className={cn(buttonVariants({ variant: "secondary", size: "sm" }), "inline-flex")}><Download size={14} /> CSV</Link> : null}
  </form>;
}
